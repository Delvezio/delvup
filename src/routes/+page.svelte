...
<header id="inizio" class="site-header wrap">
	...
	<nav aria-label="Navigazione principale">
		<a href="#lavori">Lavori <span>↗</span></a><a href="#approccio">Approccio</a><a href="#contatti"
			>Parliamone <span>↗</span></a
		>
	</nav>
	<div class="theme-switch" role="group" aria-label="Tema del sito">
		{#each modes as option (option.value)}<button
				...
				title={option.label}
				><span aria-hidden="true">{option.icon}</span><span class="theme-label">{option.label}</span
				></button
			>{/each}
	</div>
</header>
	...
	<section id="approccio" class="approach wrap" aria-labelledby="approach-title">
		...
		<div class="approach-copy">
			...
			<div class="about-copy">
				<p>
					Sono Alessandro, la persona dietro Delvup. Metto insieme design e sviluppo per dare forma
